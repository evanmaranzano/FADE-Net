...
- 5 个新创新模块已实现并通过集成测试：高频纹理增强（TEX）、频域注意力（FREQ）、年龄 MoE（MOE）、自适应三元组损失、非对称序数损失。
...
2. 设计高频纹理增强分支（Texture Branch），利用 Sobel 滤波提取面部皱纹纹理特征，增强模型对高频年龄线索的感知能力。
3. 引入频域注意力机制（Frequency-Domain Attention），通过 DCT 频域通道注意力增强特征选择。
4. 设计年龄感知混合专家头（Age MoE），通过门控网络将特征路由到不同专家，实现年龄段感知的动态预测。
5. 提出自适应三元组损失（Adaptive Triplet Loss），根据年龄差动态调整 margin，增强相邻年龄的区分能力。
6. 引入非对称序数损失（Asymmetric Ordinal Loss），对不同方向的预测误差施加不同惩罚，适应年龄估计的方向性代价。
...
本文提出 FADE-Net，一种基于 MobileNetV4-Small 的轻量级人脸年龄估计模型。模型通过高频纹理增强、频域注意力、多尺度特征融合、空间金字塔池化、年龄感知混合专家和标签分布学习等任务特定模块，在参数量精简的条件下取得了具有竞争力的年龄估计结果。由于不同文献采用的数据划分、预处理和测试协议不同，外部论文结果仅作为参考，不构成严格 SOTA 排名。
