...
NAME=fade_net_exp018_label_sigma15_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-018 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-018 fixed label sigma 1.5\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\tlabel_sigma 2.0 -> 1.5\n'
  printf 'rationale\tnarrower fixed target after adaptive broadening failed\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --label_sigma 1.5 --lambda_cdf 0.0 \
