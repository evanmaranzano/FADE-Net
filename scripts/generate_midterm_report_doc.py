...
def add_quick_facts(doc: Document) -> None:
    ...
    rows = [
        ...
        ("最佳结果", "最佳单模型 MAE 为 3.0574（seed1337）"),
        ("稳定性结果", "三次实验均值为 3.0857 ± 0.0204"),
        ("参数规模", "总参数量为 4.8415M"),
    ]
...
def add_speech(doc: Document) -> None:
    ...
    paragraphs = [
        ...
        "从目前的阶段性结果来看，模型已经取得了比较稳定的性能。最佳单模型在 seed1337 下的测试集 MAE 为 3.0574，另外两个随机种子的结果分别为 3.0951 和 3.1047，三次实验均值为 3.0857，标准差为 0.0204。模型总参数量为 4.8415M，说明该方法在保持较低参数规模的同时，能够获得有竞争力的年龄估计效果。",
...
def add_qa(doc: Document) -> None:
    ...
    qa_items = [
        ...
        (
            ...
            "答题模板：目前最直接的结果有三个。第一，最佳单模型 MAE 达到 3.0574；第二，三次不同随机种子的均值为 3.0857，标准差仅 0.0204，说明训练具有较好稳定性；第三，模型参数量只有 4.8415M，说明它不是靠堆大模型获得结果，而是在轻量化条件下取得了较好的性能。"
        ),
        (
            "Q8 你现在这个阶段还存在哪些不足？",
