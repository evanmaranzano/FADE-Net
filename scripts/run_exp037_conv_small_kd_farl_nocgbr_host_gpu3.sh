...
NAME=fade_net_exp037_conv_small_kd_farl_nocgbr_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-037 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-037 disable-CGBR Small with FaRL KD\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'baseline\tEXP-031 Small + CGBR + FaRL KD, EMA 1x Val MAE 3.2029134240\n'
  printf 'only_variable\t--disable_cgbr; all else identical to EXP-031\n'
  printf 'rationale\tTest whether CGBR helps or hurts Small under FaRL KD. EXP-033 suggested CGBR slightly hurt Medium; if Small also improves without CGBR, the 1.576M deployment baseline improves.\n'
  printf 'selection_metric\tEMA 1x Val MAE\n'
  printf 'leader_replacement_gate\tVal MAE < 3.1902 (beat EXP-031 Small baseline)\n'
