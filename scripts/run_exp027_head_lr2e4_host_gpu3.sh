...
NAME=fade_net_exp027_head_lr2e4_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-027 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-027 head learning rate 2e-4\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\thead_lr 3e-4 -> 2e-4\n'
  printf 'rationale\treduce head overfitting and validation oscillation while retaining backbone learning rate\n'
