...
NAME=fade_net_exp034_conv_small_kd_farl_input224_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-034 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-034 MobileNetV4-Conv-Small native-224 input with FaRL KD\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'baseline\tEXP-031 Small r224 weights + input 256 + FaRL KD, EMA 1x Val MAE 3.2029134240\n'
  printf 'only_variable\tinput_size 256 -> 224; backbone and e2400_r224 weights unchanged\n'
  printf 'rationale\tmatch the 98%%+ native 224x224 AFAD images and the Small r224 pretraining resolution\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --input_size 224 \
  --backbone_source timm --backbone_name mobilenetv4_conv_small \
