...
NAME=fade_net_exp041_farl_teacher_fold2_host_gpu2
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-041 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-041 FaRL ViT-B/16 teacher Fold2 (5-fold benchmark stage, raw-image protocol)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'leakage_guard\tteacher trained on Fold1 split only; Fold0 teacher (EXP-030) must NOT distill Fold2 students (its train folders contain Fold2 test folders 2,3)\n'
  printf 'only_variable\tsplit_id 0->2; all else identical to EXP-030\n'
  printf 'teacher_role\tstandalone artifact: distills Fold2 students (Small/Medium)\n'
