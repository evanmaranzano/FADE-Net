...
NAME=fade_net_exp043_farl_teacher_fold4_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-043 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-043 FaRL ViT-B/16 teacher Fold4 (5-fold benchmark stage, raw-image protocol)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'leakage_guard\tteacher trained on Fold4 split only; Fold0 teacher (EXP-030) must NOT distill Fold4 students (its train folders contain Fold4 test folders 4,5)\n'
  printf 'only_variable\tsplit_id 0->4; all else identical to EXP-030\n'
  printf 'teacher_role\tstandalone artifact: distills Fold4 students (Small/Medium)\n'
