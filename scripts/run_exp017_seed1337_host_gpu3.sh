...
NAME=fade_net_exp017_seed1337_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-017 artifacts" >&2
  exit 4
fi
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --seed 1337 --num_workers 4 --device cuda \
  > "$LOG_FILE" 2>&1
RC=$?
set -e
