...
NAME=fade_net_exp029_epochs55_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-029 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-029 cosine horizon matched to observed training length\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-028 (EXP-003 + train_crop_scale_min 0.7)\n'
  printf 'only_variable\tepochs 120 -> 55 (cosine decays to floor within observed early-stop window)\n'
  printf 'rationale\tall prior runs early-stop at epoch 36-55 with best at 16-35 while cosine is stretched over 120 epochs, so LR stays near peak and the low-LR convergence phase is never reached; matching the horizon lets the schedule complete\n'
  printf 'selection_metric\tEMA 1x Val MAE\n'
  printf 'baseline_val_mae\t3.2661378747\n'
  printf 'promotion_gate\tVal MAE < 3.2535 (pre-registered, same absolute margin as previous gate)\n'
