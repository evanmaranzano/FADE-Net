...
NAME=fade_net_exp028_crop_scale07_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-028 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-028 wider training crop scale\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\ttrain_crop_scale_min 0.8 -> 0.7\n'
  printf 'rationale\tincrease training scale diversity to improve robustness of symmetric multi-scale TTA\n'
