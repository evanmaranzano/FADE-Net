...
QUEUE_STARTED=${1:?original queue start epoch is required}
...
# EXP-009 indicates full refine weight is the unstable region. Test slower ramps first.
run_experiment exp012_cgbr_full_36_v2 --cgbr_full_epoch 36
(( $(date +%s) - QUEUE_STARTED >= MIN_SECONDS )) && exit 0
run_experiment exp013_cgbr_full_46 --cgbr_full_epoch 46
(( $(date +%s) - QUEUE_STARTED >= MIN_SECONDS )) && exit 0

# EMA changes do not alter raw optimization and are more orthogonal than lower refine weights.
run_experiment exp015_ema_09995 --ema_decay 0.9995
(( $(date +%s) - QUEUE_STARTED >= MIN_SECONDS )) && exit 0
run_experiment exp016_ema_09998 --ema_decay 0.9998
(( $(date +%s) - QUEUE_STARTED >= MIN_SECONDS )) && exit 0

# Lower-priority diagnostics if earlier runs finish unusually quickly.
