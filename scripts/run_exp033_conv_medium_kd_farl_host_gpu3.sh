...
NAME=fade_net_exp033_conv_medium_kd_farl_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-033 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-033 MobileNetV4-Conv-Medium capacity probe with FaRL KD\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'baseline\tEXP-031 MobileNetV4-Conv-Small + FaRL KD, EMA 1x Val MAE 3.2029134240\n'
  printf 'only_variable\tbackbone mobilenetv4_conv_small -> mobilenetv4_conv_medium; matching official 256px ImageNet-1K weights\n'
