...
NAME=fade_net_exp026_lambda_coarse045_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-026 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-026 coarse auxiliary weight 0.45\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\tlambda_coarse 0.3 -> 0.45\n'
  printf 'rationale\ttest the opposite direction after reducing coarse supervision to 0.15 regressed\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --label_sigma 2.0 --lambda_cdf 0.0 --lambda_coarse 0.45 \
