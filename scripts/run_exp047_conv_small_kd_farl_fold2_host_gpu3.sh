...
NAME=fade_net_exp047_conv_small_kd_farl_fold2_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-047 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-047 Fold2 Small student with FaRL KD (5-fold benchmark stage)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'stage\t5-fold CVPR2024-table stage, route A; config identical to EXP-031 except split_id 0->2 and teacher EXP-030->EXP-041 (same-fold teacher, leakage guard)\n'
