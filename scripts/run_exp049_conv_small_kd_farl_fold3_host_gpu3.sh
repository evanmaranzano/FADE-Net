...
TEACHER_CHECKPOINT="$RUNTIME_ROOT/outputs/fade_net_exp042_farl_teacher_fold3_host_gpu2/fold3/best_checkpoint.pth"
NAME=fade_net_exp049_conv_small_kd_farl_fold3_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-049 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-049 Fold3 Small student with FaRL KD (5-fold benchmark stage)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'stage\t5-fold CVPR2024-table stage, route A; config identical to EXP-031 except split_id 0->3 and teacher EXP-030->EXP-042 (same-fold teacher, leakage guard)\n'
