...
NAME=fade_net_exp030_farl_teacher_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-030 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-030 FaRL ViT-B/16 teacher (independent artifact for later distillation)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'farl_weights_sha256\t%s\n' "$FARL_WEIGHTS_SHA256"
  printf 'baseline\tEXP-029 student (Val MAE 3.2499); teacher is NOT compared against student ablations\n'
  printf 'teacher_role\tstandalone artifact: trained once, later used to distill the student\n'
