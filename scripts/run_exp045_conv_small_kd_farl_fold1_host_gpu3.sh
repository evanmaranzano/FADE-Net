...
TEACHER_CHECKPOINT="$RUNTIME_ROOT/outputs/fade_net_exp040_farl_teacher_fold1_host_gpu3/fold1/best_checkpoint.pth"
NAME=fade_net_exp045_conv_small_kd_farl_fold1_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-045 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-045 Fold1 Small student with FaRL KD (5-fold benchmark stage)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'stage\t5-fold CVPR2024-table stage, route A; config identical to EXP-031 except split_id 0->1 and teacher EXP-030->EXP-040 (same-fold teacher, leakage guard)\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --strict_official_data --split_id 1 \
