...
NAME=fade_net_exp040_farl_teacher_fold1_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-040 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-040 FaRL ViT-B/16 teacher Fold1 (5-fold benchmark stage, raw-image protocol)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'leakage_guard\tteacher trained on Fold1 split only; Fold0 teacher (EXP-030) must NOT distill Fold1 students (its train folders contain Fold1 test folders 0,1)\n'
  printf 'only_variable\tsplit_id 0->1; all else identical to EXP-030\n'
  printf 'teacher_role\tstandalone artifact: distills Fold1 students (Small/Medium)\n'
