...
NAME=fade_net_exp032_kd05_farl_input256_host_gpu3
...
cd "$PROJECT_DIR"
for path in "$PYTHON" "$AFAD_DIR" "$TIMM_WEIGHTS" "$TEACHER_CHECKPOINT" data/official/AFAD-Full.json; do
  if [ ! -e "$path" ]; then
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-032 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-032 KD weight halved relative to EXP-031\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'baseline\tEXP-031 (EXP-029 + lambda_kd 1.0), EMA 1x Val MAE 3.2029134240\n'
  printf 'only_variable\tlambda_kd 1.0 -> 0.5 (FaRL ViT-B/16 teacher frozen, Val-selected EMA weights, Val 2.9455; KD on the main distribution head only)\n'
  printf 'rationale\tlambda_kd=1.0 improved 1x and 2x substantially but degraded scale views (6x flat); halving the KD weight tests whether 1x gains survive while scale robustness recovers\n'
  printf 'selection_metric\tEMA 1x Val MAE\n'
  printf 'baseline_val_mae\t3.2499237321\n'
  printf 'promotion_gate\tVal MAE < 3.1902 (pre-registered, same absolute margin as previous gate)\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --input_size 256 --fusion_channels 96 --route_groups 8 \
  ...
  --teacher_checkpoint "$TEACHER_CHECKPOINT" --lambda_kd 0.5 \
