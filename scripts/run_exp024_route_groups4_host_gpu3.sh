...
NAME=fade_net_exp024_route_groups4_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-024 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-024 DCSR route groups 4\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\troute_groups 8 -> 4\n'
  printf 'rationale\tcoarser distribution-conditioned channel routing after 16 groups regressed\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --input_size 256 --fusion_channels 96 --route_groups 4 \
