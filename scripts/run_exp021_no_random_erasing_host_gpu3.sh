...
NAME=fade_net_exp021_no_random_erasing_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-021 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-021 disable RandomErasing\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\trandom_erasing_p 0.1 -> 0.0\n'
  printf 'rationale\tpreserve fine-grained age texture in low-resolution faces\n'
