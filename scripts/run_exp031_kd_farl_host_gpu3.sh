...
NAME=fade_net_exp031_kd_farl_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-031 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-031 FaRL teacher knowledge distillation on top of EXP-029\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'baseline\tEXP-029 (EXP-028 + cosine horizon 55 epochs), EMA 1x Val MAE 3.2499237321\n'
  printf 'only_variable\tlambda_kd 0 -> 1.0 (FaRL ViT-B/16 teacher frozen, Val-selected EMA weights, Val 2.9455; KD on the main distribution head only)\n'
  printf 'rationale\tlambda_kd=1.0 puts the KD term at the same magnitude as the main KL term; no lambda sweep is run to avoid introducing multiple variables\n'
  printf 'selection_metric\tEMA 1x Val MAE\n'
  printf 'baseline_val_mae\t3.2499237321\n'
  printf 'promotion_gate\tVal MAE < 3.2372 (pre-registered, same absolute margin as previous gate)\n'
