...
NAME=fade_net_exp044_conv_medium_kd_farl_fold1_host_gpu2
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-044 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-044 Fold1 Medium student with FaRL KD (5-fold benchmark stage)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'stage\t5-fold CVPR2024-table stage, route A (raw images); config identical to EXP-033 except split_id 0->1 and teacher EXP-030->EXP-040 (same-fold teacher, leakage guard)\n'
