...
NAME=fade_net_exp023_route_groups16_input256_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-023 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-023 DCSR route groups 16\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  printf 'code_sha256\t%s\n' "$CODE_SHA256"
  printf 'official_db_sha256\t%s\n' "$OFFICIAL_DB_SHA256"
  printf 'timm_weights_sha256\t%s\n' "$TIMM_WEIGHTS_SHA256"
  printf 'baseline\tEXP-003 seed 42 input 256\n'
  printf 'only_variable\troute_groups 8 -> 16\n'
  printf 'rationale\tfiner distribution-conditioned channel routing\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --input_size 256 --fusion_channels 96 --route_groups 16 \
