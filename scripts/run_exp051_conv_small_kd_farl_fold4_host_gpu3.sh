...
TEACHER_CHECKPOINT="$RUNTIME_ROOT/outputs/fade_net_exp043_farl_teacher_fold4_host_gpu3/fold4/best_checkpoint.pth"
NAME=fade_net_exp051_conv_small_kd_farl_fold4_host_gpu3
...
if [ -e "$OUTPUT_DIR" ] || [ -e "$LOG_FILE" ] || [ -e "$STATUS_FILE" ]; then
  echo "Refusing to overwrite existing EXP-051 artifacts" >&2
  exit 4
fi
...
{
  printf 'experiment\tEXP-051 Fold4 Small student with FaRL KD (5-fold benchmark stage)\n'
  printf 'started_at\t%s\n' "$STARTED_AT"
  ...
  printf 'stage\t5-fold CVPR2024-table stage, route A; config identical to EXP-031 except split_id 0->4 and teacher EXP-030->EXP-043 (same-fold teacher, leakage guard)\n'
  printf 'selection_metric\tEMA 1x Val MAE\n'
  printf 'fold0_reference\tEXP-031 Small Fold0: EMA 1x Val MAE 3.2029134240\n'
  printf 'test_policy\tskip_final_test during training; post-training Test TTA eval per benchmark protocol, 1x EMA enters the table\n'
...
"$PYTHON" -u src/train_fade_net.py \
  ...
  --strict_official_data --split_id 4 \
